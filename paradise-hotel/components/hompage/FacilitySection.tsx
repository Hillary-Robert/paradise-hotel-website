...
const FacilitySection = ({ facility }: { facility: facilityProp[] }) => {
  return (
    <section className="flex flex-col items-center justify-center text-center gap-10 px-6 md:px-[4rem] py-12">
      <div className="max-w-2xl">
        <h1 className="text-3xl md:text-4xl font-bold text-[#7C6A46] mb-4">
          Our Facilities
        </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 w-full max-w-6xl">
        {facilitySample.map((facility, index) => {
          ...
          return (
            <div
              key={index}
              className="flex flex-col items-center justify-center p-6 bg-white shadow-md rounded-xl transition hover:shadow-lg"
            >
              <Icon className="text-[#7C6A46] text-4xl mb-3" />
